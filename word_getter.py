...
def extract_words(novel_file):
    """从文件中提取单词"""
    words = []
    with open(novel_file, 'r', encoding='latin1') as novel:
        for line in novel:
...
def write_words_to_file(words, output_file):
    """将单词列表写入文件"""
    with open(output_file, 'w', encoding='latin1') as output:
        for word in words:
            output.write(f"{word}\n")
...
def filter_words(words, filter_file):
    ...
    with open(filter_file, 'r', encoding='latin1') as file:
...
if __name__ == "__main__":
    print("欢迎使用本程序！项目github地址请访问https://github.com/xiaolue171/Word-Getter")
    # 读取 info.txt 文件中的内容
    info_file = "info.txt"
    english_text_file = ""
    word_list_file = "word list.txt"
    filter_file = ""

    with open(info_file, 'r', encoding='latin1') as info:
        for line in info:
            line = line.strip()
